...
class AutomationGUI:
    ...
    def create_widgets(self):
        ...
        self.stop_record_btn = ttk.Button(control_frame, text="Stop Recording (F9)", 
        ...
        # Stop replay button
        self.stop_replay_btn = ttk.Button(control_frame, text="Stop Replay (F9)", 
                                         command=self.stop_replay, width=20, state=tk.DISABLED)
        ...
        instructions = (
            "Instructions:\n"
            "• Click 'Start Recording' to begin recording your actions\n"
            "• Press F9 to stop recording and auto-save, or click 'Stop Recording'\n"
            "• The first Alt+Tab you press will be automatically skipped\n"
            "• Load a recording file to replay it\n"
            "• Set repeat count and delay, then click 'Start Replay'\n"
            "• Press F9 during replay to stop playback"
        )
    ...
    def update_status(self):
        ...
        # Check if recording has stopped (e.g., via F9)
        if self.is_recording and self.recorder and not self.recorder.recording:
            # Recording was stopped (likely by F9), update GUI state
            self.is_recording = False
        ...
        if self.is_recording:
            self.status_label.config(text="Recording... Press F9 to stop and save", 
                                    foreground="red")
        elif self.is_replaying:
            self.status_label.config(text="Replaying... Press F9 to stop", 
                                    foreground="blue")
        else:
    ...
    def on_recording_stopped(self, filename=None):
        """Called when recording stops (via F9 or manual stop)."""
        self.is_recording = False
        self.record_btn.config(state=tk.NORMAL)
        self.stop_record_btn.config(state=tk.DISABLED)
        self.load_btn.config(state=tk.NORMAL)
    ...
    def start_recording(self):
        ...
        self.log("Recording started... Press F9 to stop and auto-save, or ESC to stop without saving.")
        ...
        def record_thread():
            ...
            try:
                while self.recorder.recording:
                    time.sleep(0.1)
                # Recording stopped (e.g., by F9), ensure GUI is updated if not already
    ...
    def stop_recording(self):
        ...
        # Stop the recorder (F9 would have already done this, but handle manual stop)
